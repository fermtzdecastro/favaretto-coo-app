...
export function IngresosTable({ ventasPorCanal, mes, onTotalChange }: IngresosTableProps) {
  ...
  // Ventas registradas por canal (automático)
  ...
  useEffect(() => {
    async function cargar() {
      ...
      // Cargar ventas registradas del mes y agrupar por canal
      const mesInicio = mes;
      const [anio, m] = mes.split('-').map(Number);
      const mesFin = new Date(anio, m, 1).toISOString().slice(0, 10);
  ...
  return (
    <div className="rounded-2xl bg-white p-5">
      ...
      <FilaIngreso
        label="Tienda"
        deVentas={ventasCanal.tienda}
        manual={tienda}
        setManual={setTienda}
        concepto="tienda"
      />
      <FilaIngreso
        label="Directa — Colección Actual"
        deVentas={ventasCanal.directa_coleccion}
        manual={directaColeccion}
        setManual={setDirectaColeccion}
        concepto="directa_coleccion"
      />
      <FilaIngreso
        label="Directa — Custom Made"
        deVentas={ventasCanal.directa_custom}
        manual={directaCustom}
        setManual={setDirectaCustom}
        concepto="directa_custom"
      />
      <FilaIngreso
        label="Activación"
        deVentas={ventasCanal.activacion}
        manual={activacion}
        setManual={setActivacion}
        concepto="activacion"
      />
      <FilaIngreso
        label="Otros"
        deVentas={ventasCanal.otro}
        manual={otros}
        setManual={setOtros}
        concepto="otros"
      />
